...
Debug helpers:
  ...
  --dump-json prints request/response JSON payloads and bodies
...
Run live reservation/favorite flows (requires LICENSE_PLATE):
  PYTHONPATH=src PROVIDER_ID=the_hague BASE_URL=... \
    CVP_USERNAME=... CVP_PASSWORD=... LICENSE_PLATE=AB12CD \
...
def _format_json_block(data: Any, *, sanitize: bool) -> str:
    """Return pretty-printed JSON for terminal output."""
    value = _sanitize_data(data) if sanitize else _normalize_debug_value(data)
    return json.dumps(value, indent=2, sort_keys=True, ensure_ascii=False)


def _collect_object_fields(value: Any) -> list[str]:
    """Return sorted field names from a dict or list of dicts."""
    if isinstance(value, Mapping):
...
def _resolve_credentials(args: argparse.Namespace) -> tuple[dict[str, Any], str]:
    """Resolve credentials from CLI flags, env vars, or JSON/file input."""
    if args.username and args.password:
        return {"username": args.username, "password": args.password}, "cli-flags"

    env_username = os.getenv("CVP_USERNAME")
    env_password = os.getenv("CVP_PASSWORD")
    if env_username and env_password:
        return {"username": env_username, "password": env_password}, "env"
    ...
    if credentials_json:
        return _load_credentials(credentials_json, None), "credentials-json"
    if credentials_file:
        return _load_credentials(None, credentials_file), "credentials-file"

    print(
        "Missing credentials. Use --username/--password, CVP_USERNAME/CVP_PASSWORD, "
        "or --credentials-json/--credentials-file.",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
def _format_permit_summary(permit: Any) -> str:
    """Return a compact permit summary for the default output."""
    permit_id = getattr(permit, "id", "-")
    balance = getattr(permit, "remaining_balance", "-")
    balance_unit = getattr(permit, "balance_unit", "-")
    zone_validity = getattr(permit, "zone_validity", None)
    zone_count = len(zone_validity) if isinstance(zone_validity, list) else "?"
    return f"{permit_id} | balance={balance} {balance_unit} | zone_validity={zone_count}"
...
def _print_collection(
    ...
    *,
    ...
    """Print a compact summary and optional item details for a result collection."""
    print(_format_action(label, str(count), color=color))
    if not show_items:
        return
    for item in items:
        print(f"- {item}")
...
def _print_login_info_summary(login_info: Mapping[str, Any]) -> None:
    """Print a compact overview of the login bootstrap response shape."""
    top_level_fields = ", ".join(sorted(str(key) for key in login_info.keys())) or "-"
    print(_format_action("Login info fields", top_level_fields, color="cyan"))
    for field_name in ("LoginMethods", "PermitMediaTypes", "FlowInfos", "Policy"):
        fields = _collect_object_fields(login_info.get(field_name))
        if fields:
            print(_format_action(f"{field_name} fields", ", ".join(fields), color="cyan"))


def _default_permit_media_type_id(login_info: Mapping[str, Any]) -> str | None:
    """Return the first permit media type ID from login bootstrap data."""
    permit_media_types = login_info.get("PermitMediaTypes")
    if not isinstance(permit_media_types, list) or not permit_media_types:
        return None
    first = permit_media_types[0]
    if not isinstance(first, Mapping):
        return None
    permit_media_type_id = first.get("ID")
    if permit_media_type_id is None:
        return None
    return str(permit_media_type_id)


async def _inspect_dvsportal_login_flow(
    provider: Any,
    ...
    *,
    ...
    """Print a readable DVS Portal login flow and return resolved credentials."""
    if getattr(provider, "provider_id", None) != "dvsportal":
        return dict(credentials)

    resolved_credentials = dict(credentials)
    await _run_step("Login bootstrap", provider._transport.fetch_app_env(), trace=trace)
    login_info = await _run_step(
        "Login info",
        provider._request_json(
            ...
            allow_reauth=False,
            include_auth=False,
            operation="login flow inspect",
        ),
        trace=trace,
    )
    if isinstance(login_info, Mapping):
        _print_login_info_summary(login_info)
        if show_json:
            print(_style("Login info JSON:", "cyan", bold=True))
            print(_format_json_block(login_info, sanitize=sanitize_output))
        permit_media_types = login_info.get("PermitMediaTypes")
        default_login_method = login_info.get("DefaultLoginMethod")
        if default_login_method is not None:
            print(
                _format_action(
                    "Default login method",
                    str(default_login_method),
                    color="cyan",
                )
            )
        if "permit_media_type_id" not in resolved_credentials:
            permit_media_type_id = _default_permit_media_type_id(login_info)
            if permit_media_type_id is not None:
                resolved_credentials["permit_media_type_id"] = permit_media_type_id
        chosen_media_type = resolved_credentials.get("permit_media_type_id")
        if chosen_media_type is not None:
            print(
                _format_action(
                    "Chosen permit_media_type_id",
                    str(chosen_media_type),
                    color="cyan",
                )
            )
        if isinstance(permit_media_types, list):
            print(
                _format_action(
                    "Permit media types",
                    str(len(permit_media_types)),
                    color="cyan",
                )
            )
    payload = {
        "identifier": resolved_credentials.get("username"),
        "password": resolved_credentials.get("password"),
        "permitMediaTypeID": resolved_credentials.get("permit_media_type_id"),
    }
    print(
        _format_action(
    ...
    if show_json:
        print("Login payload JSON:")
        print(_format_json_block(payload, sanitize=sanitize_output))
    ...
    def start_request(
        ...
        *,
        ...
        if self._sanitize_output:
            ...
            cookies_value = _sanitize_data(cookies) if cookies is not None else None
            ...
            cookies_value = _normalize_debug_value(cookies) if cookies is not None else None
...
class _DebugSession:
    ...
    def _collect_cookies(self, url: str) -> dict[str, str] | None:
...
def _build_trace_config(recorder: _DebugRecorder) -> aiohttp.TraceConfig:
    """Return an aiohttp trace config that logs requests without wrapping the session."""
    ...
    async def on_request_end(
        ...
        request_id = getattr(trace_config_ctx, "request_id", "0000")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--request-context",
        dest="request_context",
        help="Optional request context / municipality name, like HA passes to the provider.",
    )
    parser.add_argument(
        "--run-all",
        dest="run_all",
    ...
    parser.add_argument(
        "--show-login-flow",
        dest="show_login_flow",
        action="store_true",
        help="Print a readable login bootstrap flow before authentication.",
    )
    parser.add_argument(
        "--show-login-json",
        dest="show_login_json",
        action="store_true",
        help="Print full JSON for the login bootstrap and login payload.",
    )
    parser.add_argument(
        "--dump-limit",
        dest="dump_limit",
    ...
    parser.add_argument(
        "--verbose-results",
        dest="verbose_results",
        action="store_true",
        help="Print reservation and favorite item details in the final summary.",
    )
...
def _credential_debug_summary(credentials: Mapping[str, str]) -> str:
    """Return a safe debug summary for username/password-like credentials."""
    username = credentials.get("username", "")
    password = credentials.get("password", "")
    return (
        "Credential summary: "
        f"username_len={len(username)} "
        f"username_stripped_len={len(username.strip())} "
        f"username_equal_stripped={username == username.strip()} "
        f"password_len={len(password)} "
        f"password_stripped_len={len(password.strip())} "
        f"password_equal_stripped={password == password.strip()}"
    )


def _format_provider_login_state(provider: Any) -> str:
    """Return a concise login-state summary for debug output."""
    provider_id = getattr(provider, "provider_id", "-")
    parts = [f"provider={provider_id}"]
    if hasattr(provider, "_token"):
        token = getattr(provider, "_token", None)
        parts.append(f"token_present={bool(token)}")
    if hasattr(provider, "_session_authenticated"):
        parts.append(
            f"session_authenticated={bool(getattr(provider, '_session_authenticated', False))}"
        )
    if hasattr(provider, "_permit_media_type_id"):
        parts.append(f"permit_media_type_id={getattr(provider, '_permit_media_type_id', None)!r}")
    if hasattr(provider, "_permit_media_code"):
        permit_media_code = getattr(provider, "_permit_media_code", None)
        parts.append(f"permit_media_code_present={bool(permit_media_code)}")
    return " ".join(parts)
...
async def _run_login(
    ...
    *,
    ...
    """Run provider login and print provider-specific login state when helpful."""
    try:
        await _run_step("Login", provider.login(credentials=credentials), trace=trace)
    except AuthError:
        if getattr(provider, "provider_id", None) == "dvsportal":
            print(
                _format_action(
                    "Login state",
                    _format_provider_login_state(provider),
                    color="yellow",
                ),
                file=sys.stderr,
            )
...
async def main() -> int:
    ...
    request_context = args.request_context or os.getenv("REQUEST_CONTEXT")
    ...
    credentials, credential_source = _resolve_credentials(args)
    if extras:
    ...
    if args.debug:
        ...
        print(_credential_debug_summary(credentials), file=sys.stderr)
    ...
    client_session: aiohttp.ClientSession | _DebugSession | None = None
    if args.dump_json or args.dump_dir:
        session = aiohttp.ClientSession()
        debug_session = _DebugSession(session, recorder)
        client_session = debug_session
    elif recorder._enabled:
        client_session = aiohttp.ClientSession(trace_configs=[_build_trace_config(recorder)])
    ...
    try:
        async with Client(
            ...
            provider = await _run_step(
                "Provider load",
                client.get_provider(provider_id, request_context=request_context),
                trace=args.traceback,
            )
            if args.show_login_flow or args.show_login_json:
                credentials = await _inspect_dvsportal_login_flow(
                    provider,
                    credentials,
                    trace=args.traceback,
                    sanitize_output=args.sanitize_output,
                    show_json=args.show_login_json,
                )
            await _run_login(
                provider,
        ...
        elif isinstance(client_session, aiohttp.ClientSession):
    ...
    print(_format_action("Credential source", credential_source, color="cyan"))
    print(_format_action("Permit", _format_permit_summary(permit), color="cyan"))
    show_items = args.verbose_results or args.debug
    _print_collection(
        "Reservations",
        len(reservations),
        [
            _format_reservation(reservation, sanitize=args.sanitize_output)
    ...
    _print_collection(
        "Favorites",
        len(favorites),
        [_format_favorite(favorite, sanitize=args.sanitize_output) for favorite in favorites],
        show_items=show_items,
    )
    return 0
